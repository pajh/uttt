...
#else
void printEvCache() { 
    fprintf(stderr,"metrics are turned off\n");
}
#endif
...
static inline Evaluation evaluate(u16 board_u16, u16 free_bits)
{
    ...
    if (board_u16 >= POSS_BOARDS) {
        fprintf(stderr, "Invalid board setn to evaluate [%d]\n", board_u16);
        exit(0);
    }
...
void testRig() {
    ...
    FILE* file = fopen ("/home/paul/tictac/initboard.txt", "r");
    for (int y=0;y < 9; y++) {
    ...
    printBoard(&board,last_move, &valid_moves);
    start_time = get_gtod_clock_time();
    p0_log = true;

    //Pos m = getMove(&board, last_move, &valid_moves);
    
    Pos m = evaluateMovesMM(&board, &valid_moves);
    double tt = getElaspedTime();
    fprintf(stderr,"Move:%d %d(time %1.5f\n",m.x,m.y,tt);
